...
with open(f"{home}/IdeaProjects/hayden-bom-plugin/build_hayden_bom/BomTemplate.txt", 'r') as b:
    lines = b.readlines()
    loaded = toml.load(f"{home}/.gradle/libs.versions.toml")
    with open(f"{home}/IdeaProjects/hayden-bom-plugin/src/main/kotlin/com/hayden/haydenbomplugin/BomPlugin.kt", 'w') as out:
        for l in lines:
